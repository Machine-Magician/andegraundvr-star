...
GO
/****** Object:  StoredProcedure [dbo].[ЗначениеБонусаДляПодразделений]    Script Date: 10.02.2026 16:48:22 ******/
SET ANSI_NULLS ON
GO
...
,ABS(ДляПодразделений._fld49593) as ФактическоеЗначение
...
--and дата = '2025-10-01'
--where (сотрудник like '%сигаева%' and дата = '2025-08-01')
...
SELECT 
	...
	CASE 
		...
		WHEN Показатель COLLATE Cyrillic_General_CI_AS LIKE '%Сверхнорматив%' 
			 OR Показатель COLLATE Cyrillic_General_CI_AS LIKE '%брак%' 
			THEN 
				-- Обратная шкала для брака и сверхнорматива
				-- Формула: (1 - Факт/План) * 100
				CASE 
					WHEN ФактическоеЗначение = 0 THEN 100  -- идеально, нет брака
					WHEN ФактическоеЗначение >= ПлановоеЗначение THEN 0  -- превышение или равенство плану = 0%
					ELSE (1 - ФактическоеЗначение / ПлановоеЗначение) * 100  -- линейный расчет
				END
		ELSE 
			-- Прямая шкала для обычных показателей
			CASE 
				WHEN ФактическоеЗначение >= ПлановоеЗначение THEN 100  -- план выполнен или перевыполнен
				ELSE ФактическоеЗначение / ПлановоеЗначение * 100  -- процент выполнения
			END
	END as ПроцентВыполненияПоказателя
...
--where сотрудник like '%Гасанова Татьяна%'
...
,ABS(ЗадачиНаМесяц._fld50144) as фактическоеЗначение
 ...
 ,case 
    when ЗадачиНаМесяц._fld50143 = 0 then 0
    when COALESCE(KPIссылкаНаименование._Description, ЗадачиНаМесяц._fld50134_S) COLLATE Cyrillic_General_CI_AS LIKE '%Сверхнорматив%' 
	or COALESCE(KPIссылкаНаименование._Description, ЗадачиНаМесяц._fld50134_S) COLLATE Cyrillic_General_CI_AS LIKE '%брак%' 
        then 
            -- Обратная шкала для брака и сверхнорматива
            -- Формула: (1 - Факт/План) * 100
            case 
                when ABS(ЗадачиНаМесяц._fld50144) = 0 then 100  -- идеально, нет брака
                when ABS(ЗадачиНаМесяц._fld50144) >= ЗадачиНаМесяц._fld50143 then 0  -- превышение или равенство плану = 0%
                else (1 - ABS(ЗадачиНаМесяц._fld50144)/ЗадачиНаМесяц._fld50143) * 100  -- линейный расчет
            end
    else 
        -- Прямая шкала для обычных показателей
        case 
            when ABS(ЗадачиНаМесяц._fld50144) >= ЗадачиНаМесяц._fld50143 then 100  -- план выполнен или перевыполнен
            else (ABS(ЗадачиНаМесяц._fld50144)/ЗадачиНаМесяц._fld50143) * 100  -- процент выполнения
        end
end as ПроцентВыполненияПоказателя
...
--where (сотрудник like '%соловьева%' and дата = '2026-01-01')
...
,ПлановоеЗначение
,ФактическоеЗначение
,ПроцентВыполненияПоказателя
,ПроцентВыполненияПоказателя * Вес / 100 as Фактическое_выполение_вес
,SUM(ПроцентВыполненияПоказателя) OVER (PARTITION BY Сотрудник, Дата)/count(ПроцентВыполненияПоказателя) OVER (PARTITION BY Сотрудник, Дата) as Средневзвешенный_процент_выполнения


into #Сцепка
FROM (
    SELECT DISTINCT *  -- Убираем дубли
...
--,ПроцентВыполненияПоказателя DECIMAL(18,5)
--,фактическое_выполение_вес DECIMAL(18,5)

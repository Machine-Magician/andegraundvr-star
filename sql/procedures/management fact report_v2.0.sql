...
SELECT 
	...
	CASE 
			...
			ELSE ФактическоеЗначение / ПлановоеЗначение * 100 
		END as ПроцентВыполненияПоказателя,
		-- Защита от деления на ноль для фактическое_выполение_вес
		CASE 
			WHEN ISNULL(ПлановоеЗначение, 0) = 0 THEN 0
			ELSE (ФактическоеЗначение / ПлановоеЗначение * 100) * Вес / 100 
		END as Фактическое_выполение_вес,
...
,ЗадачиНаМесяц._fld50144 as фактическоеЗначение
 ...
 ,case 
    ...
    when COALESCE(KPIссылкаНаименование._Description, ЗадачиНаМесяц._fld50134_S) like '%Сверхнорматив%' or COALESCE(KPIссылкаНаименование._Description, ЗадачиНаМесяц._fld50134_S) like '%брак%' 
        then 
            -- Обратная шкала для брака и сверхнорматива
            -- Формула: (1 - Факт/План) * 100
            case 
                when ЗадачиНаМесяц._fld50144 = 0 then 100  -- идеально, нет брака
                when ЗадачиНаМесяц._fld50144 >= ЗадачиНаМесяц._fld50143 then 0  -- превышение или равенство плану = 0%
                else 100 -((1 - ЗадачиНаМесяц._fld50144/ЗадачиНаМесяц._fld50143) * 100)  -- линейный расчет
            end
    else 
        -- Прямая шкала для обычных показателей
        case 
            when ЗадачиНаМесяц._fld50144 >= ЗадачиНаМесяц._fld50143 then 100  -- план выполнен или перевыполнен
            else (ЗадачиНаМесяц._fld50144/ЗадачиНаМесяц._fld50143) * 100  -- процент выполнения
        end
end as ПроцентВыполненияПоказателя
...
,SUM(ПроцентВыполненияПоказателя * Вес / 100) OVER (PARTITION BY Сотрудник, Дата) as Средневзвешенный_процент_выполнения
